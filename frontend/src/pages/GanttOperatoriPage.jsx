 ...
 * Drag: pointermove globale + elementFromPoint + refs (no stale closure)
 */
...
const colorePerCantiere = id => id ? PALETTE[(id - 1) % PALETTE.length] : null

function cellaKey(tipo, id, data, turno) {
  return `${tipo}__${id}__${data}__${turno}`
}
function parseCellaKey(k) {
  const [tipo, id, data, turno] = k.split('__')
  return { tipo, id: parseInt(id), data, turno }
}

// ── Popover ───────────────────────────────────────────────────────────────────
function CellaPopover({ op, data, turno, assegnazione, cantieri, onSalva, onChiudi, rangeCelle }) {
  ...
  const isRange = rangeCelle && rangeCelle.length > 1
    ...
    <div ref={ref}
      ...
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs font-bold text-gray-800 truncate pr-2">
          {isRange
            ? `${op.nome} — ${rangeCelle.length} turni selezionati`
            : `${op.nome} — ${turno === 'M' ? 'Mattina' : 'Pomeriggio'} ${dayjs(data).format('D/M')}`}
        </p>
        <button onPointerDown={e => e.stopPropagation()} onClick={onChiudi}
          className="text-gray-300 hover:text-gray-500 flex-shrink-0"><X size={14}/></button>
      </div>
    ...
    <div className="flex flex-wrap gap-2 mt-2 px-1">
      {usati.map(c => (
        <div key={c.id} className="flex items-center gap-1.5 text-xs text-gray-600">
          <div className="w-3 h-3 rounded-sm" style={{ background: colorePerCantiere(c.id) }}/>
          {c.nome}
        </div>
      ))}
    </div>
...
function GrigliaGantt({ operatori, giorni, assMap, cantieri, onSalva, canWrite, oggi, mobile }) {
  ...
  // Tutti i dati acceduti nei listener sono in refs → no stale closure
  const dragRef   = useRef(null)   // { op, startKey, cantiereId, lavorazione }
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      const td = document.elementFromPoint(clientX, clientY)?.closest('[data-cella]')
      if (!td) return
      const { tipo, id, data, turno } = td.dataset
      const d = dragRef.current
      // Solo stessa riga
      if (tipo !== d.op.tipo || parseInt(id) !== d.op.id) return
      const key = cellaKey(tipo, id, data, turno)
  ...
  }, [canWrite]) // listener registrati una volta sola

  const startDrag = (e, op, data, turno) => {
    if (!canWrite) return
    e.preventDefault()
    const ass = assMapRef.current[cellaKey(op.tipo, op.id, data, turno)]
    dragRef.current = {
      op, startData: data, startTurno: turno,
      cantiereId: ass?.cantiere_id ?? null,
      lavorazione: ass?.lavorazione ?? null,
    }
    selRef.current = new Set([cellaKey(op.tipo, op.id, data, turno)])
    setSelKeys(new Set(selRef.current))
    setPopover(null)
  }

  const CELL_H = mobile ? 44 : 32
  const NAME_W = mobile ? 110 : 140
  ...
  const RigaOp = ({ op, zebra }) => (
    <tr className={zebra ? 'bg-gray-50/50' : ''}>
      <td className="sticky left-0 z-10 border-r-2 border-gray-200 border-b border-gray-100 px-2 py-1"
        ...
        {op.azienda
          ? <p className="text-[10px] text-gray-400 truncate">{op.azienda}</p>
          : <p className="text-[10px] text-gray-400 capitalize truncate">{op.categoria}</p>}
      </td>
      {giorni.map(d => {
        ...
        return (['M','P']).map(turno => {
          const key = cellaKey(op.tipo, op.id, dataStr, turno)
          const ass = assMap[key]
          const colore = colorePerCantiere(ass?.cantiere_id)
          const isSel = selKeys.has(key)
          const isOpen = popover?.op.id === op.id && popover?.op.tipo === op.tipo &&
          ...
          return (
            <td key={key}
              ...
              data-data={dataStr}
              data-turno={turno}
              className={`relative border border-gray-100 p-0 select-none touch-none
                ${isWeekend && !ass ? 'bg-gray-50' : ''}
                ${isOggi ? 'ring-1 ring-inset ring-steelex-orange' : ''}`}
              style={{ minWidth: mobile ? 26 : 24, height: CELL_H }}
              onPointerDown={canWrite ? e => startDrag(e, op, dataStr, turno) : undefined}>
              <div
                className={`w-full h-full flex items-center justify-center
                  ${canWrite ? 'cursor-pointer' : ''}
                  ${!ass && !isSel && canWrite ? 'hover:bg-orange-50' : ''}`}
                style={{ background: isSel ? (colore || '#fdba74') : (colore || undefined),
...
export default function GanttOperatoriPage() {
  ...
  const assMap = useMemo(() => {
    ...
    assegnazioni.forEach(a => {
      if (a.artigiano_id) map[cellaKey('artigiano', a.artigiano_id, a.data, a.turno)] = a
      if (a.utente_id)    map[cellaKey('utente', a.utente_id, a.data, a.turno)] = a
    })
    return map
  }, [assegnazioni])
    ...
    <div className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              {operatori.filter(o=>o.tipo==='artigiano').length} artigiani ·{' '}
              {operatori.filter(o=>o.tipo==='utente').length} interni
            </p>
          </div>
        </div>
      ...
      {canWrite && (
        <p className="text-xs text-gray-400 px-1">
          💡 <strong>Click</strong> per assegnare · <strong>Trascina</strong> per selezionare più turni ·
          Trascina da cella colorata per <strong>replicare</strong> il cantiere
        </p>
      )}
