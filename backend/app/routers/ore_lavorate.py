...
def lista_utenti(db: Session = Depends(get_db), utente=Depends(get_current_user)):
    """Utenti admin+amministrazione per il filtro (solo admin vede tutti)."""
    _check(utente)
    rows = db.execute(text("""
        SELECT id, nome, cognome, ruolo FROM utenti
        WHERE ruolo IN ('admin', 'amministrazione') AND attivo = TRUE
...
def lista_ore(mese: Optional[str] = None, utente_id: Optional[int] = None,
    ...
    """Registro ore del mese. Admin vede tutti, amministrazione solo le proprie."""
    _check(utente)
    where = ["1=1"]
    params = {}
...
def _riga_o_404(oid, db, utente):
    ...
    if utente.ruolo != "admin" and row["utente_id"] != utente.id:
