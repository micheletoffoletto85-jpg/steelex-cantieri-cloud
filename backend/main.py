...
def _migra():
    migrazioni = [
        ...
        """CREATE TABLE IF NOT EXISTS programmazione_settimana (
            id           SERIAL PRIMARY KEY,
            admin_id     INTEGER NOT NULL REFERENCES utenti(id),
            operativo_id INTEGER NOT NULL REFERENCES utenti(id),
            anno         INTEGER NOT NULL,
            settimana    INTEGER NOT NULL,
            giorni       JSONB DEFAULT '{}',
            creato_il    TIMESTAMPTZ DEFAULT NOW(),
        ...
        # Programmazione libera: attività fuori cantiere (ferie, corsi, permessi...)
        "ALTER TABLE assegnazioni_operatore ADD COLUMN IF NOT EXISTS tipo VARCHAR(20) NOT NULL DEFAULT 'cantiere'",
    ]
    _u = engine.url
    import psycopg2
...
app.include_router(diari.chiusura_router, prefix="/api/v1")
